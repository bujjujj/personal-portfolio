      ...
      <TiltWrapper disabled={isOpen}>
        <motion.div
          ...
          className={`w-full rounded-2xl transition-colors ${
            isOpen 
              ? "bg-white/10 border border-white/20 shadow-2xl p-2" 
              : "bg-white/5 border border-white/10 p-4 cursor-pointer hover:bg-white/10"
          }`}
        ...
        >
          <motion.div
            ...
            style={{ transformStyle: "preserve-3d" }}
            className={`w-full h-full flex flex-col rounded-xl transition-colors duration-300 ${
              isOpen 
                ? "bg-transparent p-6" 
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {projects.map((proj) => (
              <TiltWrapper key={proj.id}>
                {/* THE FIX: Added style={{ transformStyle: "preserve-3d" }} to this <a> tag 
                  so the inner layer knows how to pop out in 3D space.
                */}
                <a 
                  href={proj.link} 
                  style={{ transformStyle: "preserve-3d" }}
                  className="block group relative h-full w-full rounded-2xl bg-white/5 border border-white/10 p-4 hover:border-white/30 transition-colors"
                >
                  
                  {/* FLOATING LAYER CARD */}
                  <div 
                    style={{ transform: "translateZ(60px)", transformStyle: "preserve-3d" }} 
                    className="h-full w-full flex flex-col rounded-xl bg-black/40 backdrop-blur-xl border border-white/10 p-6 shadow-2xl transition-colors group-hover:border-white/30"
                  >
